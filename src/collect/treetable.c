...
static void  transplant     (TreeTable *table, RBNode *u, RBNode *v);
static RBNode *tree_min     (TreeTable const * const table, RBNode *n);
static RBNode *tree_max     (TreeTable const * const table, RBNode *n);
...
{
    RBNode *sentinel = NULL;
    TreeTable *table = conf->mem_calloc(1, sizeof(TreeTable));

    if (!table)
        return CC_ERR_ALLOC;

    sentinel = conf->mem_calloc(1, sizeof(RBNode));
...
enum cc_stat treetable_add(TreeTable *table, void *key, void *val)
{
    ...
    n = table->mem_alloc(sizeof(RBNode));
...
enum cc_stat treetable_remove_first(TreeTable *table, void **out)
{
    ...
    node = tree_min(table, table->root);
