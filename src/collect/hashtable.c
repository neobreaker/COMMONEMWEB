...
enum cc_stat hashtable_add(HashTable *table, void *key, void *val)
{
    ...
    hash = table->hash(key, table->key_len, table->hash_seed);
    i = hash & (table->capacity - 1);

    replace = table->buckets[i];
    ...
    new_entry = table->mem_alloc(sizeof(TableEntry));
...
{
    size_t      index;
    TableEntry *bucket = NULL;
    if (!key)
        return get_null_key(table, out);

    index = get_table_index(table, key);
...
static enum cc_stat resize(HashTable *t, size_t new_capacity)
{
    TableEntry **new_buckets = NULL;
    TableEntry **old_buckets = NULL;
    if (t->capacity == MAX_POW_TWO)
        return CC_ERR_MAX_CAPACITY;

    new_buckets = t->mem_calloc(new_capacity, sizeof(TableEntry *));
