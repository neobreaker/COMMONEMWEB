...
enum cc_stat slist_add_at(SList *list, void *element, size_t index)
{
    ...
    new = list->mem_calloc(1, sizeof(SNode));
...
enum cc_stat slist_add_all_at(SList *list1, SList *list2, size_t index)
{
    ...
    status = get_node_at(list1, index, &node, &prev);
 ...
 */
static void splice_between(SList *l1, SList *l2, SNode *base, SNode *end)
{
    if (!base)
    {
...
enum cc_stat slist_remove_last(SList *list, void **out)
{
    ...
    status = get_node_at(list, list->size - 1, &node, &prev);
...
enum cc_stat slist_sort(SList *list, int (*cmp) (void const *e1, void const *e2))
{
    ...
    if (list->size == 1)
        return CC_OK;
    status = slist_to_array(list, &elements);
...
enum cc_stat slist_iter_remove(SListIter *iter, void **out)
{
    ...
    e = unlink(iter->list, iter->current, iter->prev);
    iter->current = NULL;
    iter->index--;
...
enum cc_stat slist_zip_iter_add(SListZipIter *iter, void *e1, void *e2)
{
    ...
    SNode *new_node2 = NULL;
    if (!new_node1)
        return CC_ERR_ALLOC;

    new_node2 = iter->l2->mem_calloc(1, sizeof(SNode));
...
enum cc_stat slist_zip_iter_remove(SListZipIter *iter, void **out1, void **out2)
{
    ...
    e1 = unlink(iter->l1, iter->l1_current, iter->l1_prev);
    e2 = unlink(iter->l2, iter->l2_current, iter->l2_prev);
...
enum cc_stat slist_zip_iter_replace(SListZipIter *iter, void *e1, void *e2, void **out1, void **out2)
{
    ...
    old1 = iter->l1_current->data;
    old2 = iter->l2_current->data;
